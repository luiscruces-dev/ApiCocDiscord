...
- `/cagarse <victima>` 📱 — comando de relajo: le tira un roast al azar (tono venezolano) a quien se mandó una cagada atacando. En Discord `victima` es una mención real (`@alguien`) y hay un `motivo` opcional; en WhatsApp se escribe el nombre tal cual, se menciona con "@" en el propio texto, o se cita/responde su mensaje (el motivo va después de un `|`, ej. `/cagarse Fulano | dejó el rey en la casa`). Puro chiste entre panas, no toca nada del clan. Además, un loop de fondo revisa la guerra activa cada 10 min y manda solo un roast al grupo de WhatsApp en dos casos, con tono distinto: ataca hacia abajo (TH rival más bajo) y no saca pleno → tono de "trátame en serio" (no hay excusa de dificultad); ataca parejo o hacia arriba y saca 0 o 1 estrella → la cagada normal de siempre. Atacar hacia arriba y sacar poco nunca cuenta. Cada ataque se avisa una sola vez.
